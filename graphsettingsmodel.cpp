...
QVariant GraphSettingsModel::data(const QModelIndex &index, int role) const
 {
     ...
     if (role == Qt::DisplayRole)
     {
        ...
        if(index.column() == 3) return qVariantFromValue(plots.at(index.row())->condition());
        if(index.column() == 4) return qVariantFromValue(plots.at(index.row())->getColour());
        return QVariant();
     }
     else if(role == Qt::CheckStateRole)
     {
         if(index.column() == 5)
         {
             if(plots.at(index.row())->getEnable()) return Qt::Checked;
...
QVariant GraphSettingsModel::headerData(int section, Qt::Orientation orientation,
                                      int role) const
 {
     if (role != Qt::DisplayRole)
         return QVariant();
...
Qt::ItemFlags GraphSettingsModel::flags(const QModelIndex &index) const
 {
     ...
     if(index.column() == 4 || index.column() == 1) return QAbstractItemModel::flags(index);
     else if(index.column() == 5) return (QAbstractItemModel::flags(index) | Qt::ItemIsSelectable | Qt::ItemIsUserCheckable);
     else return QAbstractItemModel::flags(index) | Qt::ItemIsEditable | Qt::ItemIsSelectable;
 }
                               ...
                               const QVariant &value, int role)
 {
     if (index.isValid() && role == Qt::EditRole)
    {
        if(index.column() == 0)
        {
            ...
            if(QString::compare(plots[index.row()]->getGraph(), value.toString()) != 0)
            {
                emit( plotGraphChanged(plots[index.row()], plots[index.row()]->getGraph(), value.toString()) );
                /* Make the enabled be the same as other plots in same graph */
                for(int i = 0; i < plots.count(); i++)
                {
                    if(i != index.row())
                    {
                        if(QString::compare(plots[i]->getGraph(), value.toString()) == 0)
                        {
                            plots.at(index.row())->setEnable(plots.at(i)->getEnable());
                            emit dataChanged(createIndex(index.row(),5,0), createIndex(index.row(),5,0));
                        }
                    }
                }
            }
        ...
        if(index.column() == 1) plots.at(index.row())->setXaxis(value.toString());
        if(index.column() == 2) plots.at(index.row())->setYaxis(value.toString());
        if(index.column() == 3) plots.at(index.row())->setCondition(qVariantValue<Condition>(value));
        if(index.column() == 4) plots.at(index.row())->setColour(qVariantValue<QColor>(value));
        if(index.column() == 5)
        {
...
bool GraphSettingsModel::insertRows(int position, int rows, const QModelIndex &/*parent*/)
 {
     beginInsertRows(QModelIndex(), position, position+rows-1);

     for (int row = position; row < position+rows; row++)
     {
         GraphSettingsItem * gsi = new GraphSettingsItem();//agentType, condition, x, y, z, shape, colour);
         //plots.append(vsi);
...
bool GraphSettingsModel::removeRows(int position, int rows, const QModelIndex &/*parent*/)
 {
...
void GraphSettingsModel::addPlot(QString g, QString x, QString y, Condition cond, QColor c, bool e)
{
    beginInsertRows(QModelIndex(), plots.count(), plots.count());
...
QList<GraphSettingsItem *> GraphSettingsModel::getPlotsInSameGraph(QModelIndex index)
{
...
void GraphSettingsModel::switchEnabled(QModelIndex index)
{
    ...
    for(int i=0; i < plots.count(); i++)
    {
        if(QString::compare(graphName,plots.at(i)->getGraph()) == 0)
        {
            ...
            emit dataChanged(createIndex(i,index.column(),0), createIndex(i,index.column(),0));
        }
    }
}
